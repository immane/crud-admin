...
<p align="center">
  <br>
  <b>一个基于 Vue 2、Element UI 和 EasyAdmin 的配置驱动后台管理系统</b>
  <br><br>
...
- **Token 认证** — JWT 风格的 token 存储在 Cookie 中，通过 Axios 拦截器自动注入
...
- **SVG 雪碧图图标** — 通过 Webpack 配置的 SVG sprite loader 实现高性能图标使用
- **代码分割与构建优化** — chunk 分割（vendor、Element UI、commons）、preload、runtime 内联
- **导出 CSV/Excel** — 内置数据导出功能，支持自定义列标签
- **富文本编辑** — 集成 TinyMCE 组件
- **单元测试** — Jest + Vue Test Utils，带覆盖率报告
- **ESLint + TypeScript** — 代码质量和部分类型检查
- **Electron 支持** — 通过 `vue-cli-plugin-electron-builder` 实现可选的桌面端打包
...
| 文件 | 用途 | `VUE_APP_BASE_API` |
|------|---------|-------------------|
| `.env.development` | 开发服务器 | `http://localhost:9528` |
...
`vue.config.js` 通过 Vue CLI 配置 Webpack。关键设置：
- **publicPath**：生产环境为 `/admin/`
- **Dev Server**：端口 9528，集成 Mock 中间件
- **代码分割**：vendor、Element UI 和公共代码（引用次数 > 3 次）
- **预加载**：初始 chunk（排除 runtime 和 hot-update）
- **SVG Sprite**：通过 `svg-sprite-loader` 内联 SVG 图标
- **Runtime Inline**：为优化性能将 runtime 内嵌到 HTML
...
```
登录 → POST /user/login → 服务器返回 token
  ↓
token 存入 Cookie（js-cookie）
  ↓
每次请求 → Axios 拦截器添加 X-Auth-Token 请求头
  ↓
登出 → 清除 Cookie，重置 Vuex，重置路由
```
...
- **Base URL**：来自环境变量的 `VUE_APP_BASE_API`
- **超时时间**：30 秒
- **请求拦截器**：从 Cookie 注入 `X-Auth-Token` 请求头
- **响应拦截器**：检查 `response.data.code === 0` 判断成功；错误时弹出 Element UI `Message`
...
|----------|--------|-------------|
| `/login` | POST | 用户认证 |
| `/user/info` | GET | 当前用户信息 + 角色 |
| `/user/logout` | POST | 使 token 失效 |
...
│   ├── css/
│   ├── js/
│   │   ├── chunk-elementUI.{hash}.js    # Element UI（独立 chunk）
│   │   ├── chunk-libs.{hash}.js         # 其他 node_modules
│   │   ├── chunk-commons.{hash}.js      # 共享组件
│   │   └── app.{hash}.js               # 应用代码
│   └── img/
├── favicon.ico
└── index.html
...
1. 在 `.env.production` 中将 `VUE_APP_BASE_API` 设为你的 API 服务器地址
...
4. 确保服务器正确处理客户端路由——将所有路径重定向到 `index.html` 以支持 history 模式：
   ...
   - **Apache**：使用 `mod_rewrite` 的 `.htaccess`（已在 `public/.htaccess` 中提供）

---

## Electron

对于桌面端部署，本项目通过 `src/background.js` 包含了 Electron 支持。需要 `vue-cli-plugin-electron-builder`。请参考[插件文档](https://nklayman.github.io/vue-cli-plugin-electron-builder/)获取配置说明。
